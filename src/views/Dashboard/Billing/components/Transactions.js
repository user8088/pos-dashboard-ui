...
const Transactions = ({
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const url = `${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/core/transactions`;
  ...
  const fetchAccounts = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/core/accounts`, {
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
      });
  ...
  const handleDeleteTransaction = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/core/transactions/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
      });
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:8000/api'}/core/transactions`, {
        method: 'POST',
