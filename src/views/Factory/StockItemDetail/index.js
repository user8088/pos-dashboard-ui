...
export default function StockItemDetail() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const resp = await factoryStockService.showItem(id);
  ...
  if (!item) {
    return (
      <Box>
        ...
        <Button variant='outline' onClick={() => history.push('/factory/stock-management')}>Back to stock</Button>
      </Box>
    );
  }

  const cost = Number(item.last_purchase_price || item.manufacturing_cost || 0);
  ...
  return (
    <Box pt={{ base: '120px', md: '75px' }} px={{ base: '16px', md: '24px' }}>
      <Flex justify='space-between' align='center' mb='32px'>
        ...
        <Button variant='outline' onClick={() => history.push('/factory/stock-management')}>Back</Button>
      </Flex>
      ...
      <SimpleGrid columns={{ base: 1, lg: 12 }} spacing='24px'>
        ...
        <VStack align='stretch' spacing='24px' gridColumn={{ lg: '9 span' }}>
            ...
            <Card>
              ...
              <CardBody pt='16px' pb='20px' px='20px'>
                <VStack spacing='12px' align='stretch'>
                  <Box p='16px' borderRadius='8px' bg={useColorModeValue('gray.50', 'gray.800')} border='1px solid' borderColor={useColorModeValue('gray.200', 'gray.700')}>
                    <Text fontSize='xs' color='gray.500' mb='8px' fontWeight='medium' textTransform='uppercase' letterSpacing='0.5px'>Manufacturing Cost</Text>
          ...
          {/* Bottom Row: Units, Additional Info */}
